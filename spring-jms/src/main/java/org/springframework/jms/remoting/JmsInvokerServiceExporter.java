...
public class JmsInvokerServiceExporter extends RemoteInvocationBasedExporter
     ...
     * using a standard JMS {@link javax.jms.ObjectMessage} for each invocation / invocation result
     * object.
     *
     * <p>Custom implementations may generally adapt Serializables into special kinds of messages,
     * or might be specifically tailored for translating RemoteInvocation(Result)s into specific
     * kinds of messages.
     */
     ...
     *
     * <p>Switch this flag to "false" to throw an exception back to the listener container. This
     * will typically lead to redelivery of the message, which is usually undesirable - since the
     * message content will be the same (that is, still invalid).
     */
    ...
    protected void writeRemoteInvocationResult(
            Message requestMessage, Session session, RemoteInvocationResult result)
            throws JMSException {
     ...
     * RemoteInvocationResult object. It sets the response's correlation id to the request message's
    ...
    /**
     * Callback that is invoked by {@link #readRemoteInvocation} when it encounters an invalid
     * request message.
     *
     * <p>The default implementation either discards the invalid message or throws a
     * MessageFormatException - according to the "ignoreInvalidRequests" flag, which is set to
     * "true" (that is, discard invalid messages) by default.
     *
     * @param requestMessage the invalid request message
     * @return the RemoteInvocation to expose for the invalid request (typically {@code null} in
     *     case of an invalid message that will simply be ignored)
     * @throws javax.jms.JMSException in case of the invalid request supposed to lead to an

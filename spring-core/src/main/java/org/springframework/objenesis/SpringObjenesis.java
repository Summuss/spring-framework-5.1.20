...
public class SpringObjenesis implements Objenesis {
     ...
     * working at runtime, triggering the fallback code path immediately: Most importantly, this
     * means that all CGLIB AOP proxies will be created through regular instantiation via a default
     * constructor.
     */
     ...
     * @param useCache whether to use the instantiator cache (typically {@code true} but can be set
     *     to {@code false} e.g. for reloadable classes)
        ...
        try {
            ...
            if (currentWorthTrying == null) {
                ...
                if (cause instanceof ClassNotFoundException
                    ...
                    // Indicates that the chosen instantiation strategy does not work on the given
                    // JVM.
                    // Typically a failure to initialize the default
                    // SunReflectionFactoryInstantiator.
                    // Let's assume that any subsequent attempts to use Objenesis will fail as
                    // well...
                    this.worthTrying = Boolean.FALSE;
                }
            }

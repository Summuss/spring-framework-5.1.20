 ...
 * wrapped in a dynamic proxy) with additional functionality.
 *
 * @author Sam Brannen
 * @since 4.2
 * @see Annotation
...
class SynthesizedAnnotationInvocationHandler implements InvocationHandler {
    ...
    /**
     * Construct a new {@code SynthesizedAnnotationInvocationHandler} for the supplied {@link
            ...
            if (value == null) {
                String msg =
                        String.format(
                                ...
                                this.attributeExtractor.getClass().getName(),
                                attributeName,
            ...
            if (value instanceof Annotation) {
                value =
                        AnnotationUtils.synthesizeAnnotation(
                                (Annotation) value, this.attributeExtractor.getAnnotatedElement());
            } else if (value instanceof Annotation[]) {
                value =
                        AnnotationUtils.synthesizeAnnotationArray(
                                (Annotation[]) value,
    ...
    /**
     * WARNING: we can NOT use any of the {@code nullSafeHashCode()} methods in Spring's {@link
     * ObjectUtils} because those hash code generation algorithms do not comply with the
     * requirements specified in {@link Annotation#hashCode()}.
     *
    ...
    private String annotationToString() {
        ...
        Iterator<Method> iterator =
                AnnotationUtils.getAttributeMethods(annotationType()).iterator();

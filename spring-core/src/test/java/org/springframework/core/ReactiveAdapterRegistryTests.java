...
public class ReactiveAdapterRegistryTests {
    ...
    public void publisherToFlux() {
        ...
        assertEquals(
                sequence, ((Flux<Integer>) target).collectList().block(Duration.ofMillis(1000)));
    }
    ...
    public void rxObservableToPublisher() {
        ...
        assertTrue(
                "Expected Flux Publisher: " + target.getClass().getName(), target instanceof Flux);
        assertEquals(
                sequence, ((Flux<Integer>) target).collectList().block(Duration.ofMillis(1000)));
    }
    ...
    public void rxSingleToPublisher() {
        ...
        assertTrue(
                "Expected Mono Publisher: " + target.getClass().getName(), target instanceof Mono);
        assertEquals(Integer.valueOf(1), ((Mono<Integer>) target).block(Duration.ofMillis(1000)));
    }

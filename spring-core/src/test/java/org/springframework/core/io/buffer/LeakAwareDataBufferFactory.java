...
/**
 * Implementation of the {@code DataBufferFactory} interface that keep track of memory leaks. Useful
 * for unit tests that handle data buffers. Simply inherit from {@link AbstractLeakCheckingTestCase}
 * or call {@link #checkForLeaks()} in a JUnit {@link After} method yourself, and any buffers have
 * not been released will result in an {@link AssertionError}.
 *
...
public class LeakAwareDataBufferFactory implements DataBufferFactory {
     ...
     * not, then an {@link AssertionError} is thrown. Typically used from a JUnit {@link After}
     * method.
     */
                        ...
                        .map(
                                o ->
                                        o instanceof LeakAwareDataBuffer
                                                ? ((LeakAwareDataBuffer) o).getDelegate()
                                                : o)
                        .collect(Collectors.toList());

...
public class StompCommandTests {
    ...
    public void requiresDestination() throws Exception {
        for (StompCommand stompCommand : StompCommand.values()) {
            assertEquals(
                    destinationRequired.contains(stompCommand), stompCommand.requiresDestination());
        }
    }
    ...
    public void requiresSubscriptionId() throws Exception {
        for (StompCommand stompCommand : StompCommand.values()) {
            assertEquals(
                    subscriptionIdRequired.contains(stompCommand),
    ...
    public void requiresContentLength() throws Exception {
        for (StompCommand stompCommand : StompCommand.values()) {
            assertEquals(
                    contentLengthRequired.contains(stompCommand),
                    stompCommand.requiresContentLength());
        }
    }

...
public abstract class AbstractExpressionTests {
    ...
    public void evaluate(String expression, Object expectedValue, Class<?> expectedResultType) {
        ...
        if (value == null) {
            ...
            assertNull(
                    "Expression returned null value, but expected '" + expectedValue + "'",
                    expectedValue);
        }
        ...
        if (expectedValue instanceof String) {
            assertEquals(
                    "Did not get expected value for expression '" + expression + "'.",
                    expectedValue,
    ...
    public void evaluate(
        ...
        assertTrue(
                "Type of the result was not as expected.  Expected '"
                        ...
                        + resultType
                        + "'",
                expectedClassOfResult.equals(resultType));

        boolean isWritable = expr.isWritable(context);
        if (isWritable != shouldBeWritable) {
            if (shouldBeWritable) fail("Expected the expression to be writable but it is not");
            else fail("Expected the expression to be readonly but it is not");
        }
    }
    ...
    protected void evaluateAndCheckError(
        ...
        try {
            ...
            if (ex.getMessageCode() != expectedMessage) {
                assertEquals(
                        "Failed to get expected message", expectedMessage, ex.getMessageCode());
            }
            if (otherProperties != null && otherProperties.length != 0) {
                // first one is expected position of the error within the string
                int pos = ((Integer) otherProperties[0]).intValue();
                assertEquals(
                        "Did not get correct position reported in error ", pos, ex.getPosition());
                if (otherProperties.length > 1) {
                    ...
                    if (inserts.length < otherProperties.length - 1) {
                        fail(
                                "Cannot check "
                                        ...
                                        + " properties of the exception, it only has "
                                        + inserts.length
                                        + " inserts");
                    }
                    for (int i = 1; i < otherProperties.length; i++) {
                        if (otherProperties[i] == null) {
                            if (inserts[i - 1] != null) {
                                fail(
                                        "Insert does not match, expected 'null' but insert value was '"
                                                + inserts[i - 1]
                                                + "'");
                            }
                            ...
                            if (otherProperties[i] != null) {
                                fail(
                                        "Insert does not match, expected '"
                                                + otherProperties[i]

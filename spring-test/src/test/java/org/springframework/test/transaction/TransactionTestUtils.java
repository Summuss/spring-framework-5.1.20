...
/**
 * Collection of JDK 1.4+ utilities for tests involving transactions. Intended for internal use
 * within the Spring testing suite.
 *
...
public abstract class TransactionTestUtils {
    ...
    public static void assertInTransaction(boolean transactionExpected) {
        if (transactionExpected) {
            assertCondition(
                    inTransaction(), "The current thread should be associated with a transaction.");
        } else {
            assertCondition(
                    !inTransaction(),
                    "The current thread should not be associated with a transaction");
        }
    }
    ...
    /**
     * Assert the provided boolean {@code condition}, throwing {@code AssertionError} with the
     * supplied {@code message} if the test result is {@code false}.
     *

...
public abstract class JdbcTestUtils {
     ...
     *
     * <p>If the provided {@code WHERE} clause contains text, it will be prefixed with {@code "
     * WHERE "} and then appended to the generated {@code SELECT} statement. For example, if the
     * provided table name is {@code "person"} and the provided where clause is {@code "name = 'Bob'
     * and age > 25"}, the resulting SQL statement to execute will be {@code "SELECT COUNT(0) FROM
     * person WHERE name = 'Bob' and age > 25"}.
     *
     * @param jdbcTemplate the JdbcTemplate with which to perform JDBC operations
     ...
     *
     * <p>If the provided {@code WHERE} clause contains text, it will be prefixed with {@code "
     * WHERE "} and then appended to the generated {@code DELETE} statement. For example, if the
     * provided table name is {@code "person"} and the provided where clause is {@code "name = 'Bob'
     * and age > 25"}, the resulting SQL statement to execute will be {@code "DELETE FROM person
    ...
    public static int deleteFromTableWhere(
        ...
        int rowCount =
                (args.length > 0 ? jdbcTemplate.update(sql, args) : jdbcTemplate.update(sql));
        if (logger.isInfoEnabled()) {
            logger.info("Deleted " + rowCount + " rows from table " + tableName);
        }
        return rowCount;

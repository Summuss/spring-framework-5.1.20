...
abstract class BootstrapUtils {
    ...
    static BootstrapContext createBootstrapContext(Class<?> testClass) {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        String.format(
                                "Instantiating BootstrapContext using constructor [%s]",
                                constructor));
            }
            return BeanUtils.instantiateClass(
                    constructor, testClass, cacheAwareContextLoaderDelegate);
        } catch (Throwable ex) {
    ...
    private static CacheAwareContextLoaderDelegate createCacheAwareContextLoaderDelegate() {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        String.format(
                                "Instantiating CacheAwareContextLoaderDelegate from class [%s]",
                                clazz.getName()));
            }
     ...
     * BootstrapContext}, instantiate it, and provide it a reference to the {@link
     * BootstrapContext}.
     *
     * <p>If the {@link BootstrapWith @BootstrapWith} annotation is present on the test class,
     * either directly or as a meta-annotation, then its {@link BootstrapWith#value value} will be
     * used as the bootstrapper type. Otherwise, either the {@link
     * org.springframework.test.context.support.DefaultTestContextBootstrapper
     ...
     */
    static TestContextBootstrapper resolveTestContextBootstrapper(
    ...
    private static Class<?> resolveDefaultTestContextBootstrapper(Class<?> testClass)
        ...
        if (attributes != null) {
            return ClassUtils.forName(
                    DEFAULT_WEB_TEST_CONTEXT_BOOTSTRAPPER_CLASS_NAME, classLoader);
        }

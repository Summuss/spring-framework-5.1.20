...
public class PrintingResultHandler implements ResultHandler {
    ...
    /** Print the handler. */
    protected void printHandler(
            @Nullable Object handler, @Nullable HandlerInterceptor[] interceptors)
            throws Exception {
        ...
        if (mav == null || mav.getModel().size() == 0) {
            ...
            for (String name : mav.getModel().keySet()) {
                if (!name.startsWith(BindingResult.MODEL_KEY_PREFIX)) {
                    ...
                    Errors errors =
                            (Errors) mav.getModel().get(BindingResult.MODEL_KEY_PREFIX + name);
                    if (errors != null) {
    ...
    /**
     * Print the supplied cookies in a human-readable form, assuming the {@link Cookie}
     * implementation does not provide its own {@code toString()}.
     *
     * @since 4.2
     */

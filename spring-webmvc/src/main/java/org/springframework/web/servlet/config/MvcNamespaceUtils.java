...
public abstract class MvcNamespaceUtils {
    ...
    public static RuntimeBeanReference registerPathMatcher(
        ...
        if (pathMatcherRef != null) {
            ...
            parserContext
                    .getRegistry()
                    .registerBeanDefinition(PATH_MATCHER_BEAN_NAME, pathMatcherDef);
            parserContext.registerComponent(
    ...
    private static void registerBeanNameUrlHandlerMapping(
        ...
        if (!context.getRegistry()
                .containsBeanDefinition(BEAN_NAME_URL_HANDLER_MAPPING_BEAN_NAME)) {
            ...
            mappingDef
                    .getPropertyValues()
                    .add("order", 2); // consistent with WebMvcConfigurationSupport
            ...
            context.registerComponent(
                    new BeanComponentDefinition(
                            mappingDef, BEAN_NAME_URL_HANDLER_MAPPING_BEAN_NAME));
        }
    }
    ...
    private static void registerHttpRequestHandlerAdapter(
        ...
        if (!context.getRegistry().containsBeanDefinition(HTTP_REQUEST_HANDLER_ADAPTER_BEAN_NAME)) {
            ...
            context.registerComponent(
                    new BeanComponentDefinition(
                            adapterDef, HTTP_REQUEST_HANDLER_ADAPTER_BEAN_NAME));
        }
    }
    ...
    private static void registerSimpleControllerHandlerAdapter(
        ...
        if (!context.getRegistry()
            ...
            context.registerComponent(
                    new BeanComponentDefinition(
                            beanDef, SIMPLE_CONTROLLER_HANDLER_ADAPTER_BEAN_NAME));
        }
    }

    /**
     * Registers a {@code Map<String, CorsConfiguration>} (mapped {@code CorsConfiguration}s) under
     * a well-known name unless already registered. The bean definition may be updated if a non-null
    ...
    public static RuntimeBeanReference registerCorsConfigurations(
        ...
        if (!context.getRegistry().containsBeanDefinition(CORS_CONFIGURATION_BEAN_NAME)) {
            ...
            if (corsConfigurations != null) {
                corsDef.getConstructorArgumentValues()
                        .addIndexedArgumentValue(0, corsConfigurations);
            }
            context.getReaderContext()
                    .getRegistry()
                    .registerBeanDefinition(CORS_CONFIGURATION_BEAN_NAME, corsDef);
            context.registerComponent(
                    new BeanComponentDefinition(corsDef, CORS_CONFIGURATION_BEAN_NAME));

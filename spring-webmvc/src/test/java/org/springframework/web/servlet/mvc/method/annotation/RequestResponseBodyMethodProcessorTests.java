...
public class RequestResponseBodyMethodProcessorTests {
    ...
    public void supportsReturnTypeResponseBodyOnType() throws Exception {
        ...
        assertTrue(
                "Failed to recognize type-level @ResponseBody",
                processor.supportsReturnType(returnType));
    }
    ...
    public void supportsReturnTypeRestController() throws Exception {
        ...
        assertTrue(
                "Failed to recognize type-level @RestController",
                processor.supportsReturnType(returnType));
    }
    ...
    private void assertContentDisposition(
        ...
        if (expectContentDisposition) {
            assertEquals(
                    "Expected 'Content-Disposition' header. Use case: '" + comment + "'",
                    "inline;filename=f.txt",
                    header);
        } else {
            assertNull(
                    "Did not expect 'Content-Disposition' header. Use case: '" + comment + "'",
                    header);
        }

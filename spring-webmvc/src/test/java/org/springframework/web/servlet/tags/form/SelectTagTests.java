...
public class SelectTagTests extends AbstractFormTagTests {
    ...
    public void withMultiMapWithItemValueAndItemLabel() throws Exception {
        ...
        try {
            ...
            assertEquals("AT node not selected.", "selected", e.attribute("selected").getValue());
            assertEquals(
                    "AT Locale displayLanguage property not used for option label.",
                    LOCALE_AT.getDisplayLanguage(),
                    e.getData());
            ...
            assertNotNull(
                    "Option node not found with Country ISO code value [" + usa.getIsoCode() + "].",
                    e);
            assertEquals("US node not selected.", "selected", e.attribute("selected").getValue());
            assertEquals(
                    "US Locale displayLanguage property not used for option label.",
                    Locale.US.getDisplayLanguage(),
                    e.getData());

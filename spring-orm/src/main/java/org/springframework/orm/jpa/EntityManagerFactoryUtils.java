...
public abstract class EntityManagerFactoryUtils {
     ...
     * DataSourceUtils.CONNECTION_SYNCHRONIZATION_ORDER - 100 to execute EntityManager cleanup
     ...
     * @param unitName the name of the persistence unit (may be {@code null} or empty, in which case
     *     a single bean of type EntityManagerFactory will be searched for)
        ...
        if (StringUtils.hasLength(unitName)) {
            ...
            for (String candidateName : candidateNames) {
                EntityManagerFactory emf =
                        (EntityManagerFactory) beanFactory.getBean(candidateName);
                if (emf instanceof EntityManagerFactoryInfo
                        && unitName.equals(
                                ((EntityManagerFactoryInfo) emf).getPersistenceUnitName())) {
                    return emf;
                }
            }
            ...
            // Find unique EntityManagerFactory bean in the context, falling back to parent
            // contexts.
    ...
    public static EntityManager doGetTransactionalEntityManager(
        ...
        if (emHolder != null) {
            if (synchronizedWithTransaction) {
                if (!emHolder.isSynchronizedWithTransaction()) {
                    if (TransactionSynchronizationManager.isActualTransactionActive()) {
                        ...
                        try {
                            ...
                            logger.debug(
                                    "Could not join transaction because none was actually active",
                                    ex);
                        }
                    }
                ...
                if (emHolder.isTransactionActive() && !emHolder.isOpen()) {
                    ...
                    // with no synchronized EntityManager having been requested by application code
                    // before.
        ...
        if (!synchronizedWithTransaction) {
            try {
                ...
                // JPA 2.1 API available but method not actually implemented in persistence
                // provider:
        ...
        try {
            ...
            if (synchronizedWithTransaction) {
                ...
                TransactionSynchronizationManager.registerSynchronization(
                        new TransactionalEntityManagerSynchronization(
                                emHolder, emf, transactionData, true));
                emHolder.setSynchronizedWithTransaction(true);
            } else {
                // Unsynchronized - just scope it for the transaction, as demanded by the JPA 2.1
                // spec...
                TransactionSynchronizationManager.registerSynchronization(
            ...
            // Unexpected exception from external delegation call -> close EntityManager and
            // rethrow.
     ...
     * @return the corresponding DataAccessException instance, or {@code null} if the exception
    ...
    /**
     * Callback for resource cleanup at the end of a non-JPA transaction (e.g. when participating in
     * a JtaTransactionManager transaction), fully synchronized with the ongoing transaction.
     *
    ...
    private static class TransactionalEntityManagerSynchronization
        ...
        protected void cleanupResource(
                EntityManagerHolder resourceHolder,

...
public class EncoderHttpMessageWriterTests {
    ...
    public void useNegotiatedMediaType() {
        ...
        writer.write(
                Mono.just("body"), forClass(String.class), TEXT_PLAIN, this.response, NO_HINTS);
    ...
    public void useDefaultMediaTypeCharset() {
        ...
        assertEquals(
                new MediaType("text", "html", UTF_8), this.response.getHeaders().getContentType());
        ...
        assertFalse(
                (Boolean)
                        method.invoke(
                                ...
                                new MediaType(
                                        TEXT_PLAIN,
                                        Collections.singletonMap("streaming", "false"))));
    ...
    private HttpMessageWriter<String> getWriter(
            Flux<DataBuffer> encodedStream, MimeType... mimeTypes) {
        List<MimeType> typeList = Arrays.asList(mimeTypes);
        when(this.encoder.getEncodableMimeTypes()).thenReturn(typeList);
        when(this.encoder.encode(any(), any(), any(), this.mediaTypeCaptor.capture(), any()))
                .thenReturn(encodedStream);

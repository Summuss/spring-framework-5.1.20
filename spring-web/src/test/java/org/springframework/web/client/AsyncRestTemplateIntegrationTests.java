...
public class AsyncRestTemplateIntegrationTests extends AbstractMockWebServerTestCase {
    ...
    public void getNoContentTypeHeader() throws Exception {
        ...
        assertArrayEquals(
                "Invalid content", helloWorld.getBytes("UTF-8"), responseEntity.getBody());
    }
    ...
    public void headForHeadersCallbackWithLambdas() throws Exception {
        ListenableFuture<HttpHeaders> headersFuture = template.headForHeaders(baseUrl + "/get");
        headersFuture.addCallback(
                result -> assertTrue("No Content-Type header", result.containsKey("Content-Type")),
                ex -> fail(ex.getMessage()));
        waitTillDone(headersFuture);
    }
    ...
    public void postForLocationCallbackWithLambdas() throws Exception {
        ...
        ListenableFuture<URI> locationFuture =
                template.postForLocation(baseUrl + "/{method}", entity, "post");
        locationFuture.addCallback(
                result -> assertEquals("Invalid location", expected, result),
                ex -> fail(ex.getMessage()));
        waitTillDone(locationFuture);
    }
        ...
        resultFuture.addCallback(
                new ListenableFutureCallback<ResponseEntity<Void>>() {
                    ...
                    public void onSuccess(ResponseEntity<Void> result) {
                        assertEquals(
                                "Invalid location", expected, result.getHeaders().getLocation());
                        assertFalse(result.hasBody());
                    }

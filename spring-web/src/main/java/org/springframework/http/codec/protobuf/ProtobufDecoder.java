...
public class ProtobufDecoder extends ProtobufCodecSupport implements Decoder<Message> {
     ...
     *
     * <p>By default in 5.1 this is set to 64K. In 5.2 the default for this limit is set to 256K.
     *
    ...
    public Mono<Message> decodeToMono(
        ...
        return DataBufferUtils.join(inputStream, getMaxMessageSize())
                .map(
                        dataBuffer -> {
                            try {
                                ...
                                builder.mergeFrom(
                                        CodedInputStream.newInstance(buffer),
                                        this.extensionRegistry);
                                return builder.build();
                            } catch (IOException ex) {
                                throw new DecodingException(
                                        "I/O error while parsing input stream", ex);
                            } catch (Exception ex) {
                                throw new DecodingException(

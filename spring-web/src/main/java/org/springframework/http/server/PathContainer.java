...
/**
 * Structured representation of a URI path whose elements have been pre-parsed into a sequence of
 * {@link Separator Separator} and {@link PathSegment PathSegment} elements.
 *
 * <p>An instance of this class can be created via {@link #parsePath(String)}. Each {@link
 * PathSegment PathSegment} exposes its structure decoded safely without the risk of encoded
 * reserved characters altering the path or segment structure and without path parameters for path
 * matching purposes.
 *
...
public interface PathContainer {
    ...
    /**
     * Extract a sub-path from the given start offset (inclusive) into the element list and to the
    ...
    interface PathSegment extends Element {
        ...
        /**
         * Return the path segment value to use for pattern matching purposes. By default this is
         * the same as {@link #value()} but may also differ in sub-interfaces (e.g. decoded,
         * sanitized, etc.).
         */
        String valueToMatch();

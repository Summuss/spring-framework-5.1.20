...
public class JamonPerformanceMonitorInterceptor extends AbstractMonitoringInterceptor {
    ...
    /**
     * Set whether to track all invocations that go through this interceptor, or just invocations
    ...
    /**
     * Always applies the interceptor if the "trackAllInvocations" flag has been set; else just
     * kicks in if the log is enabled.
     *
    ...
    /**
     * Wraps the invocation with a JAMon Monitor and writes the current performance statistics to
     * the log (if enabled).
     *
    ...
    protected Object invokeUnderTrace(MethodInvocation invocation, Log logger) throws Throwable {
        ...
        try {
            ...
            if (!this.trackAllInvocations || isLogEnabled(logger)) {
                writeToLog(
                        logger,
                        "JAMon performance statistics for method [" + name + "]:\n" + monitor);
            }
        }
    }

    /**
     * Count the thrown exception and put the stack trace in the details portion of the key. This
     * will allow the stack trace to be viewed in the JAMon web application.
     */
    protected void trackException(MonKey key, Throwable ex) {
        ...
        MonitorFactory.add(
                new MonKeyImp(MonitorFactory.EXCEPTIONS_LABEL, stackTrace, "Exception"), 1);
    }
}

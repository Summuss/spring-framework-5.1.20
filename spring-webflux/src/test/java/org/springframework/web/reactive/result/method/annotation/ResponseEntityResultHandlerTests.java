...
public class ResponseEntityResultHandlerTests {
    ...
    public void handleResponseEntityWithExistingResponseHeaders() throws Exception {
        ...
        assertEquals(1, exchange.getResponse().getHeaders().size());
        assertEquals(
                MediaType.APPLICATION_JSON, exchange.getResponse().getHeaders().getContentType());
        assertResponseBodyIsEmpty(exchange);
    }
    ...
    private void assertConditionalResponse(
        ...
        if (lastModified.isAfter(Instant.EPOCH)) {
            assertEquals(
                    1, exchange.getResponse().getHeaders().get(HttpHeaders.LAST_MODIFIED).size());
            assertEquals(
                    lastModified.toEpochMilli(),
                    exchange.getResponse().getHeaders().getLastModified());
        }
    }

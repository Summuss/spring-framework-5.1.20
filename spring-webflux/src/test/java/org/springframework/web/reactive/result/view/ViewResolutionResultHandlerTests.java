...
public class ViewResolutionResultHandlerTests {
    ...
    private void testSupports(MethodParameter returnType, boolean supports) {
        ...
        if (supports) {
            assertTrue(
                    "return type [" + returnType + "] should be supported",
                    resultHandler.supports(handlerResult));
        } else {
            assertFalse(
                    "return type [" + returnType + "] should not be supported",
                    resultHandler.supports(handlerResult));
        }
    }
    ...
    private void assertResponseBody(MockServerWebExchange exchange, String responseBody) {
        StepVerifier.create(exchange.getResponse().getBody())
                .consumeNextWith(
                        buf ->
                                assertEquals(
                                        responseBody, DataBufferTestUtils.dumpString(buf, UTF_8)))
                .expectComplete()
                .verify();
    }

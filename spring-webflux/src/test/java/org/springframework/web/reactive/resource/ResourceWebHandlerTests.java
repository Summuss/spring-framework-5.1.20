...
public class ResourceWebHandlerTests {
    ...
    public void getResource() throws Exception {
        ...
        assertEquals(
                headers.getLastModified() / 1000, resourceLastModifiedDate("test/foo.css") / 1000);
    ...
    public void getResourceNoCache() throws Exception {
        ...
        assertEquals(
                response.getHeaders().getLastModified() / 1000,
                resourceLastModifiedDate("test/foo.css") / 1000);
    ...
    public void getResourceWithHtmlMediaType() throws Exception {
        ...
        assertTrue(headers.containsKey("Last-Modified"));
        assertEquals(
                headers.getLastModified() / 1000, resourceLastModifiedDate("test/foo.html") / 1000);
        assertEquals("bytes", headers.getFirst("Accept-Ranges"));
        assertEquals(1, headers.get("Accept-Ranges").size());
    }
    ...
    public void getResourceFromAlternatePath() throws Exception {
        ...
        assertTrue(headers.containsKey("Last-Modified"));
        assertEquals(
                headers.getLastModified() / 1000,
                resourceLastModifiedDate("testalternatepath/baz.css") / 1000);
        assertEquals("bytes", headers.getFirst("Accept-Ranges"));
        assertEquals(1, headers.get("Accept-Ranges").size());
        assertResponseBody(exchange, "h1 { color:red; }");
    }
    ...
    public void testInvalidPath() throws Exception {
        ...
        when(resource.getFilename())
                .thenThrow(new AssertionError("Resource should not be resolved"));
                ...
                .expectErrorSatisfies(
                        err -> {
                            ...
                            assertEquals(
                                    ...
                                    ((ResponseStatusException) err).getStatus());
                        })
                .verify(TIMEOUT);
    }
    ...
    public void processPath() {
        ...
        assertEquals(
                "/foo/bar",
                this.handler.processPath((char) 1 + " / " + (char) 127 + " // foo/bar"));
    ...
    private void resourceNotFound(HttpMethod httpMethod) {
        ...
        StepVerifier.create(mono)
                .consumeErrorWith(ex -> assertNotSame(exceptionRef.get(), ex))
                .verify();
    }
    ...
    public void partialContentInvalidRangeHeader() {
        ...
        assertEquals(
                HttpStatus.REQUESTED_RANGE_NOT_SATISFIABLE, exchange.getResponse().getStatusCode());
        assertEquals("bytes", exchange.getResponse().getHeaders().getFirst("Accept-Ranges"));
    }
        ...
        StepVerifier.create(exchange.getResponse().getBody())
                .consumeNextWith(
                        buf ->
                                assertEquals(
                                        responseBody,
                                        DataBufferTestUtils.dumpString(

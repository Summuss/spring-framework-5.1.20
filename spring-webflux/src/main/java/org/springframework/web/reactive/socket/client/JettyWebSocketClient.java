...
public class JettyWebSocketClient implements WebSocketClient, Lifecycle {
     ...
     *
     * <p><strong>Note: </strong> Use of this constructor implies the Jetty {@code WebSocketClient}
     * is externally managed and hence {@link Lifecycle} methods of this class are not delegated to
     * it.
     */
    ...
    private Mono<Void> executeInternal(URI url, HttpHeaders headers, WebSocketHandler handler) {
        ...
        return Mono.fromCallable(
                        () -> {
                            ...
                            return this.jettyClient.connect(
                                    jettyHandler, url, request, upgradeListener);
                        })
                .then(completionMono);
    }

    private Object createHandler(
            URI url, WebSocketHandler handler, MonoProcessor<Void> completion) {

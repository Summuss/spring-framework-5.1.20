 ...
 * body.
 *
 * <p><strong>NOTE:</strong> When given access to a {@link ClientResponse}, through the {@code
 * WebClient} {@link WebClient.RequestHeadersSpec#exchange() exchange()} method, you must always use
 * one of the body or toEntity methods to ensure resources are released and avoid potential issues
 * with HTTP connection pooling. You can use {@code bodyToMono(Void.class)} if no response content
 * is expected. However keep in mind that if the response does have content, the connection will be
 * closed and will not be placed back in the pool.
 *
 * @author Brian Clozel

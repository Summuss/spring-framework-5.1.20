...
public abstract class JmxUtils {
     ...
     * found. Logs a warning if more than one {@code MBeanServer} found, returning the first one
     * from the list.
     *
     ...
     *     {@code null}, all registered MBeanServers are considered. If the empty String is given,
     *     the platform MBeanServer will be returned.
     * @return the {@code MBeanServer} if found
     * @throws MBeanServerNotFoundException if no {@code MBeanServer} could be found
    ...
    /**
     * Create a {@code String[]} representing the argument signature of a method. Each element in
     * the array is the fully qualified class name of the corresponding argument in the methods
     * signature.
     *
     ...
     *
     * <p>When using strict casing, a JavaBean property with a getter method such as {@code
     * getFoo()} translates to an attribute called {@code Foo}. With strict casing disabled, {@code
     * getFoo()} would translate to just {@code foo}.
     *
     ...
     * resource being exposed on the supplied {@link ObjectName}. This can be used to provide a
     * unique {@link ObjectName} for each distinct instance of a particular bean or class. Useful
     * when generating {@link ObjectName ObjectNames} at runtime for a set of managed resources
     * based on the template value supplied by a {@link
    ...
    public static ObjectName appendIdentityToObjectName(
            ObjectName objectName, Object managedResource) throws MalformedObjectNameException {

        Hashtable<String, String> keyProperties = objectName.getKeyPropertyList();
        keyProperties.put(
                IDENTITY_OBJECT_NAME_KEY, ObjectUtils.getIdentityHexString(managedResource));
     ...
     *
     * <p>This implementation returns the superclass for a CGLIB proxy and the class of the given
     * bean else (for a JDK proxy or a plain bean class).
     *

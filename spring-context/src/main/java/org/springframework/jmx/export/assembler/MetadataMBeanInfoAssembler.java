...
public class MetadataMBeanInfoAssembler extends AbstractReflectiveMBeanInfoAssembler
    ...
    /**
     * Create a new {@code MetadataMBeanInfoAssembler} which needs to be configured through the
     * {@link #setAttributeSource} method.
     */
    ...
    /**
     * Set the {@code JmxAttributeSource} implementation to use for reading the metadata from the
     * bean class.
     *
     * @see org.springframework.jmx.export.annotation.AnnotationJmxAttributeSource
     */
    ...
    protected String getDescription(Object managedBean, String beanKey) {
        ManagedResource mr =
                obtainAttributeSource().getManagedResource(getClassToExpose(managedBean));
        return (mr != null ? mr.getDescription() : "");
    }

    /**
     * Creates a description for the attribute corresponding to this property descriptor. Attempts
     * to create the description using metadata from either the getter or setter attributes,
     * otherwise uses the property name.
     */
    @Override
    ...
    /**
     * Reads {@code MBeanParameterInfo} from the {@code ManagedOperationParameter} attributes
     * attached to a method. Returns an empty array of {@code MBeanParameterInfo} if no attributes
     * are found.
     */
    @Override
     ...
     * Specifically, adds the {@code currencyTimeLimit}, {@code persistPolicy}, {@code
     * persistPeriod}, {@code persistLocation} and {@code persistName} descriptor fields if they are
    ...
    protected void populateMBeanDescriptor(Descriptor desc, Object managedBean, String beanKey) {
        ...
        if (mr == null) {
            throw new InvalidMetadataException(
                    "No ManagedResource attribute found for class: "
                            + getClassToExpose(managedBean));
        }
    ...
    /**
     * Adds descriptor fields from the {@code ManagedAttribute} attribute or the {@code
     * ManagedMetric} attribute to the attribute descriptor.
     */
    @Override
    ...
    private void populateAttributeDescriptor(
        ...
        String persistPolicy =
                resolveStringDescriptor(gma.getPersistPolicy(), sma.getPersistPolicy());
     ...
     * descriptor. Specifically, adds the {@code currencyTimeLimit} descriptor field if it is
     * present in the metadata.
     */
    @Override
     ...
     * use that value. In the event that both values are non-negative, we use the greater of the
     * two. This method can be used to resolve any {@code int} valued descriptor where there are two
     * possible values.
     *

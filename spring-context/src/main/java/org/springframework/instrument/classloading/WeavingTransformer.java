...
public class WeavingTransformer {
    ...
    /**
     * Apply transformation on a given class byte definition. The method will always return a
     * non-null byte array (if no transformation has taken place the array content will be identical
     * to the original one).
     *
    ...
    public byte[] transformIfNecessary(
        ...
        for (ClassFileTransformer cft : this.transformers) {
            try {
                byte[] transformed =
                        cft.transform(this.classLoader, internalName, null, pd, result);
                if (transformed != null) {
                    result = transformed;
                }

...
public class CacheTestUtils {
    ...
    public static void assertCacheMiss(Object key, Cache... caches) {
        for (Cache cache : caches) {
            assertNull(
                    "No entry in " + cache + " should have been found with key " + key,
                    cache.get(key));
        }
    }
    ...
    public static void assertCacheHit(Object key, Object value, Cache... caches) {
        for (Cache cache : caches) {
            ...
            assertNotNull(
                    "An entry in " + cache + " should have been found with key " + key, wrapper);
            assertEquals(
                    "Wrong value in " + cache + " for entry with key " + key, value, wrapper.get());
        }
    }
}

...
public class MutablePropertyValuesTests extends AbstractPropertyValuesTests {
    ...
    public void testChangeOfOneField() {
        ...
        PropertyValues changes = pvs2.changesSince(pvs);
        assertTrue(
                "changes are empty, not of length " + changes.getPropertyValues().length,
    ...
    public void streamContainsPropertyValue() {
        ...
        assertThat(
                pvs.stream()
                        .anyMatch(pv -> "foo".equals(pv.getName()) && "bar".equals(pv.getValue())),
                is(true));
        assertThat(
                pvs.stream()
                        .anyMatch(pv -> "bar".equals(pv.getName()) && "foo".equals(pv.getValue())),
                is(false));
    }
